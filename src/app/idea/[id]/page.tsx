...
export default function IdeaDetailPage() {
  ...
  const conversationStarters = useMemo(() => {
    if (!idea) return [];
    const seed = idea.id.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0);
    return getStarters(idea.moods, seed);
  }, [idea]);
    ...
    <div className="min-h-[100dvh] flex flex-col bg-bg-primary">
      ...
      >
        ...
        {conversationStarters.length > 0 && (
          <div className="bg-accent-secondary/8 border border-accent-secondary/15 rounded-2xl p-4 mb-6">
            <p className="text-xs font-bold text-accent-secondary uppercase tracking-wider mb-3 flex items-center gap-1.5">
            ...
            <ul className="flex flex-col gap-2.5">
              {conversationStarters.map((q, i) => (
                <li key={i} className="text-sm text-text-secondary leading-relaxed flex gap-2">
                  <span className="text-accent-secondary/50 font-bold shrink-0">{i + 1}.</span>
                  {q}
                </li>
              ))}
            </ul>
